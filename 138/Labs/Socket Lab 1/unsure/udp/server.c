...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		response_len = recvfrom(sockfd, buffer, BUFF_SIZE, 0, (struct sockaddr *)&server_addr, &addr_len);
		...
		if (sendto(sockfd, new, strlen(new), 0, (struct sockaddr *)&server_addr, addr_len) < 0)
			ERR("Error Responding");
	}
}
